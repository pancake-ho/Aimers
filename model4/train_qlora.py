...
def parse_args():
    ...
    p.add_argument("--model_dir", "--base_model_path", dest="model_dir", type=str, default="./model_q4_train")
    ...
    p.add_argument("--num_train_epochs", "--epochs", dest="num_train_epochs", type=int, default=3)
    p.add_argument(
    ...
    if dataset_config:
        try:
            ...
            if "BuilderConfig" not in msg or "not found" not in msg:
                raise
            print(
                f"[WARN] Dataset config '{dataset_config}' is unavailable for {dataset_name}. "
...
def format_example(example: Dict[str, str], tokenizer: AutoTokenizer) -> Dict[str, str]:
    ...
    if "messages" in example:
        ...
        if not messages:
            raise ValueError("Empty/invalid 'messages' field in dataset example.")
        try:
