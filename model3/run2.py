...
def build_layer_config(model):
    """
    Hybrid-bit policy:
    - MLP projections (gate/up/down) -> 8-bit
    - Everything else remains at global bits=4
    - q_proj/k_proj/v_proj are intentionally left at 4-bit for vLLM fused QKV compatibility
    """
    layer_config = {}
...
print(f"[AutoRound INFO] Hybrid-bit overrides applied to {len(layer_config)} MLP modules.")
